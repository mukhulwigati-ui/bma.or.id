...
function EmbeddedZakatCalculator({
  ...
  if (activeTab === 'penghasilan') {
    ...
    isWajib = berat >= 85;
    totalZakat = isWajib
      ? Math.round((berat * HARGA_EMAS) * 0.025)
      : 0;
  }
    ...
    <div className="border border-gray-200 bg-white overflow-hidden my-4 shadow-sm rounded-xl">
        ...
        <button
          ...
          className={`flex-1 py-3 text-center border-b-2 transition cursor-pointer ${
            activeTab === 'penghasilan'
              ? 'text-emerald-900 border-emerald-900 bg-white'
      ...
      <div className="p-4 space-y-4 text-left">
        {activeTab !== 'emas' ? (
          <>
            <div>
              <label className="text-xs sm:text-sm font-medium text-slate-600 block mb-1.5">
              ...
              <input
                ...
                value={input1}
                onChange={(e) =>
                  setInput1(formatRupiah(e.target.value))
                }
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                value={input2}
                onChange={(e) =>
                  setInput2(formatRupiah(e.target.value))
                }
              />
            </div>
          </>
        ...
        <div className="bg-emerald-50/60 border border-emerald-100 p-4 text-center space-y-2 rounded-xl">
          <span className="text-xs sm:text-sm font-semibold text-slate-500 uppercase tracking-wide block">
            Estimasi Wajib Zakat Anda
          </span>

          <span className="text-xl sm:text-2xl font-extrabold text-emerald-900 block">
          ...
          <button
            ...
            onClick={() =>
              onApplyAmount(
                totalZakat.toLocaleString('id-ID')
              )
            }
        ...
        <p className="text-[11px] sm:text-xs text-slate-400 text-center">
          Nilai di atas adalah estimasi. Zakat wajib ditunaikan
          jika harta mencapai nishab dan haul.
        </p>
      </div>
    </div>
  );
...
const DonationFormFields = ({
  ...
  return (
    <div className="space-y-4 text-left">
      <div>
        <label className="text-xs sm:text-sm font-extrabold text-slate-900 block mb-2">
      ...
      {isLoggedIn ? (
        hasPhone ? (
          <div className="bg-emerald-50 border border-emerald-200/80 p-3.5 rounded-xl flex items-center justify-between">
            ...
            <span className="text-[11px] bg-emerald-600 text-white font-bold px-2.5 py-1 rounded-full shrink-0">
              Siap Donasi
            </span>
          </div>
        ) : (
          <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl space-y-3">
            <div>
              ...
              <p className="text-[11px] text-amber-700">
                Lengkapi nomor WhatsApp Anda sekali ini saja untuk
                pengiriman kuitansi dan laporan donasi.
              </p>
            </div>
...
export default function CampaignDetailClient({
  ...
  const [inlinePhone, setInlinePhone] =
    useState('');
  ...
  const [isMobileFormOpen, setIsMobileFormOpen] =
    useState(false);
  ...
  const handleInlineSavePhone =
    async () => {
      ...
      try {
        ...
        alert(
          'Nomor WhatsApp berhasil dicatat!'
        );
      } catch (
        err: any
      ) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="text-center py-20 text-slate-500 font-medium text-sm sm:text-base">
  ...
  if (!program) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="text-center py-20 text-red-500 font-medium text-sm sm:text-base">
  ...
  const whatsappText =
    `${program?.title || 'Program Kebaikan'}` +
    `\n\n` +
    `${canonicalShareUrl}`;
        ...
        <div className="bg-white p-4 sm:p-6 shadow-sm border border-gray-200/90 space-y-4 rounded-xl">
          ...
          <div className="space-y-2 pt-1">
            ...
            <div className="w-full bg-gray-100 h-2.5 overflow-hidden shadow-inner rounded-full">
              <div
                className="bg-[#e91e63] h-full transition-all duration-500"
                style={{
                  width:
                    `${percentage}%`,
                }}
                    ...
                    .map(
                      (
                        ...
                        <div
                          ...
                          className="bg-gray-50 border border-gray-200/80 p-4 space-y-2.5 rounded-xl"
                        >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-3">
          ...
          <div className="relative w-full max-w-md bg-white p-5 space-y-4 z-10 shadow-2xl border border-gray-200 text-left rounded-2xl">
            ...
            <div className="grid grid-cols-3 gap-2.5 pt-1">
              ...
              >
                <svg
                  className="w-5 h-5 fill-current text-slate-900 mt-0.5"
                  viewBox="0 0 24 24"
                >
